...
@MainActor
final class CCHUDPanel: NSPanel {
    var onUserGeometryChangeEnded: ((NSWindow) -> Void)?

    func finishUserGeometryChange() {
        onUserGeometryChangeEnded?(self)
    }
}
...
final class CCHUDController {
    ...
    static let windowIdentifier = NSUserInterfaceItemIdentifier("LingoFloat.CCHUD")
    private static let frameAutosaveName = "LingoFloat.CCHUD.v2"
    ...
    @ObservationIgnored private var appliedGeometry: GeometrySettings?
    ...
    private var geometrySettings: GeometrySettings {
        GeometrySettings(widths: store.ccHUDWidthFractionByDisplay,
                         heights: store.ccHUDHeightByDisplay,
                         centers: store.ccHUDCenterXFractionByDisplay,
                         offsets: store.ccHUDBottomOffsetByDisplay,
                         display: store.targetDisplayUUID)
    }
    ...
    private func ensureWindow() {
        ...
        let content = CCHUDContentView(
            rootView: CCHUDView(
                ...
                onClear: { [weak self] in self?.clearContext() }
            )
        )
        ...
        p.minSize = NSSize(width: 280, height: SettingsStore.ccHUDHeightRange.lowerBound)
        ...
        // Registering an autosave name does not restore a frame. The per-display
        // settings are the same source of truth used by the geometry sliders.
        positionAtBottomOfTargetScreen(p)

        p.onUserGeometryChangeEnded = { [weak self] window in
            self?.persistGeometry(of: window)
            window.saveFrame(usingName: Self.frameAutosaveName)
        }

        self.host = content.host
        self.window = p
        appliedGeometry = geometrySettings
    ...
    private func positionAtBottomOfTargetScreen(_ w: NSWindow) {
        ...
        let height = min(store.ccHUDHeight(forDisplayUUID: screen.wc_displayUUID), frame.height)
        let centerXFraction = store.ccHUDCenterXFraction(forDisplayUUID: screen.wc_displayUUID)
        let bottomOffset = store.ccHUDBottomOffset(forDisplayUUID: screen.wc_displayUUID)
        let width = min(frame.width * widthFraction, frame.width)
        let desiredCenterX = frame.minX + frame.width * centerXFraction
        let x = min(max(desiredCenterX - width / 2, frame.minX), frame.maxX - width)
        let y = min(max(frame.minY + bottomOffset, frame.minY), frame.maxY - height)
    ...
    private func persistGeometry(of window: NSWindow) {
        guard let screen = window.screen ?? Self.screen(containing: window.frame),
              let uuid = screen.wc_displayUUID else { return }

        let screenFrame = screen.frame
        guard screenFrame.width > 0 else { return }
        store.setCCHUDWidthFraction(window.frame.width / screenFrame.width, forDisplayUUID: uuid)
        store.setCCHUDHeight(window.frame.height, forDisplayUUID: uuid)
        store.setCCHUDCenterXFraction(
            (window.frame.midX - screenFrame.minX) / screenFrame.width,
            forDisplayUUID: uuid
        )
        store.setCCHUDBottomOffset(window.frame.minY - screenFrame.minY, forDisplayUUID: uuid)
        // These settings came from this exact frame. Do not feed them back into
        // a reposition on NSScreen.main, which may be a different display.
        appliedGeometry = geometrySettings
    }

    private static func screen(containing windowFrame: NSRect) -> NSScreen? {
        NSScreen.screens.max {
            $0.frame.intersection(windowFrame).width * $0.frame.intersection(windowFrame).height
                < $1.frame.intersection(windowFrame).width * $1.frame.intersection(windowFrame).height
        }
    }
        ...
        withObservationTracking { [weak self] in
            ...
            Task { @MainActor [weak self] in
                ...
                if let w = self.window {
                    ...
                    if self.isShown {
                        if self.geometrySettings != self.appliedGeometry {
                            self.positionAtBottomOfTargetScreen(w)
                            self.appliedGeometry = self.geometrySettings
                        }
