...
final class WindowFrameController {
    ...
    private static func autosaveName(for window: NSWindow) -> String? {
        ...
        if id.contains("com_apple_SwiftUI_Settings") || id.contains("NSPreferencesPanel") {
            return "LingoFloat.SettingsWindow"
        }
        // The compact cinema controller replaced the much larger chat layout.
        // A versioned autosave name prevents upgrades from restoring the old
        // 900+ point window frame and surrounding the new UI with empty space.
        return "LingoFloat.CinemaWindow.v2"
    }
    ...
    private func resetAll() {
        ...
        for key in defaults.dictionaryRepresentation().keys
            where key.hasPrefix("NSWindow Frame LingoFloat.") {
...
extension Notification.Name {
    ...
    static let resetWindowFrames = Notification.Name("lingofloat.resetWindowFrames")
}
