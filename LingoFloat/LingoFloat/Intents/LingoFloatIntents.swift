...
struct ToggleRecognitionIntent: AppIntent {

    static var title: LocalizedStringResource = "Start or Stop Transcription"
    static var description = IntentDescription(
        "Start listening to system audio, or stop the active transcription session.",
        categoryName: "LingoFloat"
    )
    ...
    @MainActor
    func perform() async throws -> some IntentResult {
        IntentActions.shared.toggleRecognition?()
        return .result()
    }
}
...
struct SetWindowOpacityIntent: AppIntent {
    ...
    static var description = IntentDescription(
        "Set the opacity (transparency) of one of the LingoFloat windows. Pass a percentage from 30 to 100.",
        categoryName: "LingoFloat"
    )
...
/// Surfaces intents in the Shortcuts app under a LingoFloat section.
struct LingoFloatShortcuts: AppShortcutsProvider {
    static var appShortcuts: [AppShortcut] {
        AppShortcut(
            intent: ToggleRecognitionIntent(),
            phrases: [
                "Start or stop \(.applicationName)",
                "Toggle \(.applicationName) transcription",
            ],
            shortTitle: "Start / Stop Transcription",
