...
///          which calls `liveBubble(for:)` to find the open bubble and
    ///          marks it final after punctuation, then starts a new UI row).
///
///   * **Splitter** — in sentence-aware mode, a completed sentence moves
///     to the context row as soon as the next sentence starts. `maxChars`
///     is only a safety cap for pathological run-on text without punctuation.
///
...
final class BubbleSplitter {
    ...
    func process(_ caption: Caption, config: Config) -> [Caption] {
        ...
        // --- step 3: close a complete sentence, or enforce the safety cap ---
        let sentenceBoundary = config.sentenceAware
            ? Self.lastCompletedSentenceBoundary(in: displayText)
            : nil
        let overflowBoundary = displayText.count > config.maxChars
            ? Self.findSplitPosition(
                in: displayText,
        ...
        if let splitPos = sentenceBoundary ?? overflowBoundary {
    ...
    /// Returns an index into overlong text at which to apply the safety cut.
    /// A multilingual sentence boundary wins; whitespace and finally the
    /// hard cap are fallbacks only when a single run-on sentence is too long.
    static func findSplitPosition(in text: String, maxChars: Int, sentenceAware: Bool) -> Int? {
        ...
        if sentenceAware {
            if let pos = lastIndex(in: text, lower: 0, upperExclusive: upper, where: { sentenceTerminators.contains($0) }) {
                return pos + 1
            }
        }
    ...
    /// Boundary after the most recent complete sentence that already has
    /// non-whitespace text following it. A terminator at the very end stays
    /// in the current row until the next sentence begins (or silence closes
    /// it), preventing an empty current subtitle from flashing on screen.
    static func lastCompletedSentenceBoundary(in text: String) -> Int? {
        var candidate: Int?
        let characters = Array(text)

        for index in characters.indices where sentenceTerminators.contains(characters[index]) {
            var boundary = index + 1
            while boundary < characters.count,
                  sentenceClosingCharacters.contains(characters[boundary]) {
                boundary += 1
            }
            let tail = characters[boundary...]
    ...
    static func endsWithSentenceTerminator(_ text: String) -> Bool {
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard let last = trimmed.last else { return false }
        if sentenceTerminators.contains(last) { return true }
        if sentenceClosingCharacters.contains(last) {
            let withoutCloser = String(trimmed.dropLast())
                .trimmingCharacters(in: .whitespacesAndNewlines)
            return withoutCloser.last.map(sentenceTerminators.contains) ?? false
        }
        return false
    }

    /// Latin, CJK, Arabic and Indic sentence endings used by the languages
    /// exposed in the recognition picker.
