...
actor WhisperEngine: TranscriptionEngine {
    ...
    /// Local model folder resolved by the managed installer before startup.
    ...
    /// Load a model that the managed installer has already downloaded and
    /// validated. Keeping network I/O outside this actor prevents the mic
    /// and system engines from racing to download the same files.
    func prepare() async {
        ...
        let config = WhisperKitConfig(
            model: modelName,
            downloadBase: try? WhisperModelInstaller.downloadRoot(),
    ...
    private static func modelLooksComplete(at folder: String) -> Bool {
        WhisperModelInstaller.isCompleteModelFolder(
    ...
    private func decode(finalize: Bool) async {
        ...
        let text = Self.sanitizeTranscript(results.map(\.text).joined(separator: " "))
    ...
    /// Whisper occasionally emits this sentinel when a decoded phrase ends in
    /// silence. It is model metadata, not speech, so it must never reach the
    /// transcript or translation pipeline.
