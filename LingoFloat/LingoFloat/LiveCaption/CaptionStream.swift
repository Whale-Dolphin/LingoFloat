...
final class CaptionStream {
    ...
    func start() async {
        ...
        // 1) Resolve the engine and install the selected local model before
        // asking for capture permission. A first-run user should be able to
        // complete the model download even when macOS requires a relaunch
        // after granting Screen Recording access.
        ...
        var modelFolder: URL? = settingsStore?.whisperModelFolderURL
        let tokenizerFolder: URL? = settingsStore?.whisperTokenizerFolderURL

        if engineKind == .whisper {
            let needsManagedInstall = modelFolder.map {
                !WhisperModelInstaller.isCompleteModelFolder($0)
                    || !WhisperModelInstaller.folderMatches($0, model: model)
            } ?? true

            if needsManagedInstall {
                state = .loadingModel(
                    progress: 0,
                    message: "Downloading \(model.displayName) (\(model.displaySize))…"
                )
                do {
                    let installedFolder = try await WhisperModelInstaller.install(model: model) { [weak self] fraction in
                        let percent = Int((fraction * 100).rounded())
                        self?.state = .loadingModel(
                            progress: fraction,
                            message: "Downloading \(model.displayName)… \(percent)%"
                        )
                    }
                    modelFolder = installedFolder
                    settingsStore?.whisperModelFolderPath = installedFolder.path
                    settingsStore?.whisperTokenizerFolderPath = nil
                } catch {
                    state = .error("Could not download \(model.displayName): \(error.localizedDescription)\n\nCheck your internet connection and press Start to retry.")
                    return
                }
            }

            guard modelFolder != nil else {
                state = .error("Could not resolve the local WhisperKit model.")
                return
            }
        }

        // 2) Microphone permission, only when microphone capture is enabled.
        // System audio uses Core Audio Process Tap and has its own
        // NSAudioCaptureUsageDescription prompt; ScreenCaptureKit access is
        // neither required nor an honest proxy for this capability.
        ...
        // 3) Engine load. Skip the mic engine entirely when mic is disabled.
        ...
        do {
            ...
            await teardown()
            state = .error("System audio capture failed: \(String(describing: error)). Allow LingoFloat to record system audio in System Settings → Privacy & Security, then relaunch and retry.")
            return
        }
    ...
    /// Save the current transcript to History and start from an empty display.
    /// Loaded models and cloud connections stay alive, but their current phrase
    /// buffers are dropped so the next subtitle starts a clean context.
    func clearContext() {
        newSession()
    }

    private func resetRecognitionContext() {
        bubbleSplitter.resetAll()
        if let micEngine {
            Task { await micEngine.reset() }
        }
        if let systemEngine {
            Task { await systemEngine.reset() }
        }
    }
    ...
    /// Background loop that finalises an idle bubble after
    /// `bubbleSilenceBreakSec`. In sentence-aware mode it only closes text
    /// that already ends in punctuation, so a dramatic mid-sentence pause
    /// continues in the same row. Polls once a second — coarse enough to be
    /// cheap and fine enough that the perceived delay is bounded.
    ...
    private func checkBubbleSilence() {
        ...
        for source in [CaptionSource.microphone, CaptionSource.system] {
            ...
            // In sentence-aware mode, an ordinary pause is not a text
            // boundary. Keep accumulating until punctuation arrives so a
            // dramatic pause in a film does not create a mysterious cut in
            // the middle of one sentence.
            if settingsStore?.bubbleSentenceAware ?? SettingsStore.defaultBubbleSentenceAware,
               let caption = captions.first(where: { $0.id == live.id }),
               !BubbleSplitter.endsWithSentenceTerminator(caption.text) {
                continue
            }
            // Find the live bubble in the array and mark it final. The engine
            // can keep emitting under its own id; the splitter retains its
            // consumed offset while minting a fresh UI row for new text.
