...
struct HotkeyDescriptor: Codable, Equatable, Sendable {
    ...
    /// The two watch-mode actions ship with conservative global defaults.
    /// They remain fully editable in Settings → Hotkeys. Less central
    /// screenshot / legacy Main HUD actions stay opt-in.
    static let defaultRecognitionToggle = HotkeyDescriptor(
        keyCode: UInt32(kVK_ANSI_T),
        modifiers: UInt32(controlKey | optionKey)
    )
    static let defaultCCHUDToggle = HotkeyDescriptor(
