...
@Observable
@MainActor
final class LingoFloatAppDelegate: NSObject, NSApplicationDelegate {
    ...
    let history  = LingoFloatAppDelegate.makeHistory()
...
@main
struct LingoFloatApp: App {

    @NSApplicationDelegateAdaptor(LingoFloatAppDelegate.self) var appDelegate

    var body: some Scene {

        // Main HUD — the compact live-caption controller. Always visible on
...
final class HotkeyCoordinator {
    ...
    init(
        ...
        IntentActions.shared.toggleRecognition = { [weak self] in self?.handleRecognitionToggle() }
    ...
    private func applyAndObserve() {
        ...
        registerRecognitionToggle(descriptor: store.recognitionToggleHotkey)
        ...
        withObservationTracking {
            ...
            _ = store.recognitionToggleHotkey
    ...
    private func registerRecognitionToggle(descriptor: HotkeyDescriptor) {
        guard !descriptor.isEmpty else {
            manager.unregister(id: "recognitionToggle")
            return
        }
        do {
            try manager.register(id: "recognitionToggle", descriptor: descriptor) { [weak self] in
                self?.handleRecognitionToggle()
            }
        } catch {
            showAlert(title: "Couldn't bind Start / Stop Transcription", message: error.localizedDescription)
        }
    }
    ...
    func handleRecognitionToggle() {
        guard let stream, !stream.state.isBusy else { return }
...
extension Notification.Name {
    ...
    static let toggleMainHUDRequested = Notification.Name("lingofloat.toggleMainHUDRequested")
}
