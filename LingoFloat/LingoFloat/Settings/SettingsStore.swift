...
final class SettingsStore {
    ...
    static let defaultBubbleMaxChars: Int = 600
    ...
    static let bubbleMaxCharsRange: ClosedRange<Int> = 120 ... 2000
    ...
    var whisperModel: WhisperModel {
        didSet {
            ...
            if oldValue != whisperModel,
               let folder = whisperModelFolderURL,
               !WhisperModelInstaller.folderMatches(folder, model: whisperModel) {
                whisperModelFolderPath = nil
                whisperTokenizerFolderPath = nil
            }
        }
    }
    ...
    /// Resolved folder for the managed WhisperKit model. The app writes this
    /// after first-run download so later sessions can start offline.
    ...
    /// Global shortcut for starting or stopping transcription.
    var recognitionToggleHotkey: HotkeyDescriptor {
        didSet { Self.persist(recognitionToggleHotkey, key: Keys.recognitionToggleHotkey) }
    }
    ...
    /// Horizontal centre of the CC strip as a 0...1 fraction across each
    /// display. This is updated when the user drags the strip directly.
    var ccHUDCenterXFractionByDisplay: [String: Double] {
        didSet { Self.persist(ccHUDCenterXFractionByDisplay, key: Keys.ccHUDCenterXFractionByDisplay) }
    }
    ...
    /// Default horizontal position: centred on the display.
    static let defaultCCHUDCenterXFraction: Double = 0.5
    ...
    static let ccHUDWidthFractionRange: ClosedRange<Double> = 0.18 ... 0.98
    static let ccHUDHeightRange: ClosedRange<Double> = 64 ... 600
    static let ccHUDCenterXFractionRange: ClosedRange<Double> = 0 ... 1
    static let ccHUDBottomOffsetRange: ClosedRange<Double> = 0 ... 4000
    ...
    func ccHUDCenterXFraction(forDisplayUUID uuid: String?) -> Double {
        guard let uuid, let stored = ccHUDCenterXFractionByDisplay[uuid] else {
            return Self.defaultCCHUDCenterXFraction
        }
        return min(max(stored, Self.ccHUDCenterXFractionRange.lowerBound), Self.ccHUDCenterXFractionRange.upperBound)
    }
    ...
    func setCCHUDCenterXFraction(_ value: Double, forDisplayUUID uuid: String) {
        var dict = ccHUDCenterXFractionByDisplay
        dict[uuid] = min(max(value, Self.ccHUDCenterXFractionRange.lowerBound), Self.ccHUDCenterXFractionRange.upperBound)
        ccHUDCenterXFractionByDisplay = dict
    }
    ...
    /// Master switch for the auto-translate feature. On by default because
    /// LingoFloat's primary workflow is foreign-language video playback.
    ...
    init() {
        ...
        self.recognitionToggleHotkey = Self.loadCodable(key: Keys.recognitionToggleHotkey) ?? .defaultRecognitionToggle
        ...
        self.ccHUDCenterXFractionByDisplay = Self.loadCodable(key: Keys.ccHUDCenterXFractionByDisplay) ?? [:]
        ...
        self.translationTargetLanguage = Language(rawValue: storedTarget) ?? .zh
    ...
    static func loadAppPresenceMode() -> AppPresenceMode {
        ...
        return AppPresenceMode(rawValue: raw) ?? .both
    }
    ...
    private enum Keys {
        ...
        static let ccHUDVisible             = "LingoFloat.settings.ccHUDVisible"
        static let bubbleMaxChars           = "LingoFloat.settings.bubbleMaxChars"
        static let bubbleSilenceBreakSec    = "LingoFloat.settings.bubbleSilenceBreakSec"
        static let bubbleSentenceAware      = "LingoFloat.settings.bubbleSentenceAware"
        static let bubbleFontSize           = "LingoFloat.settings.bubbleFontSize"
        static let targetDisplayUUID        = "LingoFloat.settings.targetDisplayUUID"
        static let deepgramAPIKey           = "deepgram-api-key"
        static let elevenLabsAPIKey         = "elevenlabs-api-key"
        static let transcriptionEngine      = "LingoFloat.settings.transcriptionEngine"
        static let screenshotHotkey         = "LingoFloat.settings.screenshotHotkey"
        static let recognitionToggleHotkey  = "LingoFloat.settings.recognitionToggleHotkey"
        static let mainHUDToggleHotkey      = "LingoFloat.settings.mainHUDToggleHotkey"
        static let ccHUDToggleHotkey        = "LingoFloat.settings.ccHUDToggleHotkey"
        static let screenshotTarget         = "LingoFloat.settings.screenshotTarget"
        static let whisperModel             = "LingoFloat.settings.whisperModel"
        static let whisperModelFolderPath   = "LingoFloat.settings.whisperModelFolderPath"
        static let whisperTokenizerFolderPath = "LingoFloat.settings.whisperTokenizerFolderPath"
        static let whisperVocabularyHint    = "LingoFloat.settings.whisperVocabularyHint"
        static let ccHUDWidthFractionByDisplay = "LingoFloat.settings.ccHUDWidthFractionByDisplay"
        static let ccHUDHeightByDisplay        = "LingoFloat.settings.ccHUDHeightByDisplay"
        static let ccHUDCenterXFractionByDisplay = "LingoFloat.settings.ccHUDCenterXFractionByDisplay"
        static let ccHUDBottomOffsetByDisplay  = "LingoFloat.settings.ccHUDBottomOffsetByDisplay"
        static let ccBackgroundColorHex        = "LingoFloat.settings.ccBackgroundColorHex"
        static let ccPreviousLineColorHex      = "LingoFloat.settings.ccPreviousLineColorHex"
        static let ccCurrentLineColorHex       = "LingoFloat.settings.ccCurrentLineColorHex"
        static let ccTranslationColorHex       = "LingoFloat.settings.ccTranslationColorHex"
        static let appearance                  = "LingoFloat.settings.appearance"
        static let accentColor                 = "LingoFloat.settings.accentColor"
        static let micBubbleColor              = "LingoFloat.settings.micBubbleColor"
        static let systemBubbleColor           = "LingoFloat.settings.systemBubbleColor"
        static let activeChatID                = "LingoFloat.settings.activeChatID"
        static let translationEnabled          = "LingoFloat.settings.translationEnabled"
        static let translationTargetLanguage   = "LingoFloat.settings.translationTargetLanguage"
        static let appPresenceMode             = "LingoFloat.settings.appPresenceMode"
    }
}
