...
struct CCHUDSection: View {
    ...
    var body: some View {
        SectionShell(descriptor: descriptor) {
            ...
            SettingsCard(
                title: "Size",
                footer: "Drag the strip itself to move it. Resize from any edge or corner; position and size are remembered per display."
            ) {
                if displays.isEmpty {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            sliderRow(
                title: "Horizontal",
                value: Binding(
                    get: { store.ccHUDCenterXFraction(forDisplayUUID: display.uuid) },
                    set: { store.setCCHUDCenterXFraction($0, forDisplayUUID: display.uuid) }
                ),
                range: SettingsStore.ccHUDCenterXFractionRange,
                trailing: "\(Int(store.ccHUDCenterXFraction(forDisplayUUID: display.uuid) * 100))%"
            )
            ...
            sliderRow(
                ...
                range: 0 ... max(400, display.frame.height - SettingsStore.ccHUDHeightRange.lowerBound),
