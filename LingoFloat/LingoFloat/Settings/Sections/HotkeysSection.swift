    ...
    var body: some View {
        ...
        SectionShell(descriptor: descriptor) {
            ...
            SettingsCard(
                ...
                footer: "These shortcuts work system-wide while LingoFloat is running, even when the app is in the background. Press a combination with at least one modifier (⌃ ⌥ ⌘ ⇧). Press Esc to cancel recording. Use Clear to unbind. Note that some apps (Zoom, Teams) reserve their own shortcuts — pick a combo they don't use. Unbound actions can still be triggered via Shortcuts, App Intents, and Stream Deck."
            ) {
                VStack(spacing: 0) {
                    hotkeyRow(
                        title: "Start / Stop Transcription",
                        subtitle: "Toggle live system-audio transcription. Default: ⌃⌥T.",
                        binding: $store.recognitionToggleHotkey
                    )
                    SettingsRowDivider()
                    hotkeyRow(
                        title: "Show / Hide Subtitle HUD",
                        subtitle: "Toggle the floating movie subtitle strip. Default: ⌃⌥H.",
                    ...
                    HStack {
                        ...
                        Button("Clear all") {
                            ...
                            store.recognitionToggleHotkey = .empty
