...
private struct AboutSeeForYourselfCard: View {
    ...
    private let repoURL = URL(string: "https://github.com/Whale-Dolphin/LingoFloat")!
    ...
    var body: some View {
        SettingsCard(
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                Button(action: { NSWorkspace.shared.open(repoURL) }) {
                    HStack(spacing: 9) {
                        Image(systemName: "chevron.left.forwardslash.chevron.right")
                            .font(.system(size: 13, weight: .semibold))
                        Text("Open LingoFloat repository on GitHub")
                            .font(.callout.weight(.semibold))
                        Spacer(minLength: 0)
                ...
                .accessibilityLabel("Open the LingoFloat repository on GitHub")
            }
        }
    }
}
    ...
    var body: some View {
        SettingsCard(title: nil, footer: nil) {
            AboutLinkRow(
                ...
                tint: .orange,
                title: "Thank the upstream author",
                subtitle: "This fork starts from WhisperCaption. Donations on the next page go to its original author."
            ) {
...
private struct AboutDiagnosticsCard: View {
    ...
    var body: some View {
        SettingsCard(
            title: "Diagnostics",
            footer: "Logs use the OSLog subsystem `com.whaledolphin.lingofloat`. Inspect them in Console.app or via `log show --predicate 'subsystem == \"com.whaledolphin.lingofloat\"' --info --debug --last 5m`."
        ) {
            HStack {
                SettingsRowLabel(
                    title: "Reveal logs in Console",
                    subtitle: "Opens Console.app for inspection of LingoFloat's log entries."
                )
                Spacer()
                Button("Open Console") {
