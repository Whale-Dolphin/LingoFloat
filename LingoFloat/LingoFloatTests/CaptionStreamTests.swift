...
struct CaptionStreamTests {
    ...
    @Test("Clear preserves the old transcript in History and empties the active display")
    func clearContextPreservesHistory() throws {
        let temp = try TempHistory.make()
        defer { temp.cleanup() }
        let settings = SettingsStore()
        let stream = CaptionStream()
        stream.attach(settings: settings, history: temp.store)
        let priorID = stream.activeSession.id
        stream.appendScreenshot(pngData: CaptionFixtures.tinyPNG, label: "before clear")

        stream.clearContext()

        #expect(stream.captions.isEmpty)
        #expect(stream.activeSession.id != priorID)
        #expect(temp.store.load(id: priorID)?.captions.first?.text == "before clear")
    }
