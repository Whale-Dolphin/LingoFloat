...
struct ElevenLabsEngineTests {
    ...
    @Test("Auto omits a fixed language and returns the detected language")
    func autoDetectsCommittedLanguage() async throws {
        let suiteName = "ElevenLabsEngineTests.auto.\(UUID().uuidString)"
        ...
        let openedURL = try #require(factory.openHistory.first?.url)
        let query = try #require(URLComponents(url: openedURL, resolvingAgainstBaseURL: false)?.queryItems)
        #expect(query.first(where: { $0.name == "language_code" }) == nil)
        #expect(query.first(where: { $0.name == "include_language_detection" })?.value == "true")

        transport.enqueueReceiveText(
            "{\"message_type\":\"committed_transcript_with_timestamps\",\"text\":\"こんにちは\",\"language_code\":\"jpn\"}"
        )
        let captionTask = Task { () -> Caption? in
            for await caption in engine.captions { return caption }
            return nil
        }
        let caption = try #require(await captionTask.value)
        #expect(caption.language == .ja)
