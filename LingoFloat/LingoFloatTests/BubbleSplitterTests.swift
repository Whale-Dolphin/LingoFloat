...
struct BubbleSplitterTests {
    ...
    @Test("A completed sentence moves to context before the safety cap")
    func naturalSentenceBoundarySplitsEarly() throws {
        let splitter = BubbleSplitter()
        let input = Caption(
            id: UUID(),
            source: .system,
            text: "This sentence is complete. The next one is live",
            language: .en,
            isFinal: false,
            startedAt: baseDate,
            updatedAt: baseDate
        ...
        let out = splitter.process(
            input,
            config: .init(maxChars: 600, sentenceAware: true)
        )

        #expect(out.count == 2)
        #expect(try #require(out.first).text == "This sentence is complete.")
        #expect(try #require(out.last).text == "The next one is live")
    }

    @Test("Chinese and Japanese full stops preserve complete sentences")
    func cjkSentenceBoundaries() {
        #expect(BubbleSplitter.lastCompletedSentenceBoundary(in: "第一句话。第二句话") == 5)
        #expect(BubbleSplitter.lastCompletedSentenceBoundary(in: "最初の文です。次の文") == 7)
        #expect(BubbleSplitter.endsWithSentenceTerminator("这是完整的一句。"))
        #expect(BubbleSplitter.endsWithSentenceTerminator("「終わりました。」"))
    }

    @Test("A terminator at the end does not create an empty current row")
    func trailingTerminatorWaitsForContinuation() {
        #expect(BubbleSplitter.lastCompletedSentenceBoundary(in: "Complete sentence.") == nil)
        #expect(BubbleSplitter.lastCompletedSentenceBoundary(in: "完整的一句。   ") == nil)
    }
