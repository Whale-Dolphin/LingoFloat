...
struct DeepgramEngineTests {
    ...
    @Test("Auto uses Nova multilingual detection and normalizes regional language codes")
    func autoDetectsRegionalLanguageCode() async throws {
        let suiteName = "DeepgramEngineTests.auto.\(UUID().uuidString)"
        ...
        #expect(query.first(where: { $0.name == "language" })?.value == "multi")

        transport.enqueueReceiveText(
            "{\"type\":\"Results\",\"is_final\":true,\"channel\":{\"alternatives\":[{\"transcript\":\"hello\",\"languages\":[\"en-US\"]}]}}"
        )
